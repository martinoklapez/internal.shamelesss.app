...
SELECT * FROM (VALUES
  ...
  ('rate_app_stars', 'Rate App (Stars)', 'Star rating + Continue; 1–3★ feedback modal. options jsonb object: show_skip_button / hide_skip_button (hide wins for hiding Skip), require_feedback_text (default true) / allow_empty_feedback_submit (empty Send allowed).', ARRAY['conversion']::text[], '{}'::jsonb),
  ('tracking_permission', 'Tracking Permission', 'ATT mockup (Allow / Ask App Not to Track).', ARRAY['conversion']::text[], '{}'::jsonb),
  ('profile_image', 'Profile Image', 'Profile photo picker (skippable). options jsonb: object — testimonial_avatars (preferred) or avatars (http(s) URLs, max 8); show_testimonial_marquee (default true) or hide_testimonial_marquee to hide marquee/final_reviews; skipable etc.', ARRAY['conversion']::text[], '{}'::jsonb),
  ...
  ('data_consents', 'Data consents', 'Consent checkboxes + Accept all + Next. Conversion only. options jsonb: object with consents[] (id, title, description?, required?, learn_more?), accept_all_label, next_button_label — not a top-level array.', ARRAY['conversion']::text[], '{"consents":[{"id":"privacy","title":"Privacy & data use","description":"Plain text body under the title.","required":true,"learn_more":{"label":"Learn more","url":"https://example.com/privacy"}}],"accept_all_label":"Accept All","next_button_label":"Next"}'::jsonb),
