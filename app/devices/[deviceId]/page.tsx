...
interface TransferDeviceOption {
  id: string
  name: string
...
export default async function DevicePage({ params, searchParams }: DevicePageProps) {
  ...
  const allManagerIds = Array.from(
    new Set((allDevicesData || []).map((item) => item.manager_id).filter((id): id is string => Boolean(id)))
  )
  ...
  const transferDeviceOptions: TransferDeviceOption[] =
    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
        ...
        <DeviceDetails
          ...
          transferDeviceOptions={transferDeviceOptions}
        />
      </div>
    </div>
  )
